...
static void gfx_s2dex_bg_copy(uObjBg* bg) {
    ...
    */
    ...
    if ((bool)gfx_check_image_signature((char*)data)) {
        std::shared_ptr<Ship::Texture> tex = std::static_pointer_cast<Ship::Texture>(Ship::Window::GetInstance()->GetResourceManager()->LoadResourceProcess((char*)data));
        texFlags = tex->Flags;
        ...
        switch (opcode) {
                ...
                if ((i & 1) != 1) {
                    if (gfx_check_image_signature(imgData) == 1) {
                        std::shared_ptr<Ship::Texture> tex =
                            std::static_pointer_cast<Ship::Texture>(Ship::Window::GetInstance()->GetResourceManager()->LoadResourceProcess(imgData));
            ...
            case G_SETTIMG_OTR_HASH: {
                ...
                std::shared_ptr<Ship::Texture> texture =
                    std::static_pointer_cast<Ship::Texture>(Ship::Window::GetInstance()->GetResourceManager()->LoadResourceProcess(GetResourceNameByCrc(hash)));
                if (texture != nullptr) {
            ...
            case G_SETTIMG_OTR_FILEPATH: {
                ...
                std::shared_ptr<Ship::Texture> texture =
                    std::static_pointer_cast<Ship::Texture>(Ship::Window::GetInstance()->GetResourceManager()->LoadResourceProcess(fileName));
                if (texture != nullptr) {
                    texFlags = texture->Flags;
                    rawTexMetadata.width = texture->Width;
                    rawTexMetadata.height = texture->Height;
...
void gfx_register_blended_texture(const char* name, uint8_t* mask, uint8_t* replacement) {
    ...
    if (gfx_check_image_signature(reinterpret_cast<char*>(replacement))) {
        Ship::Texture* tex =
            std::static_pointer_cast<Ship::Texture>(Ship::Window::GetInstance()->GetResourceManager()->LoadResourceProcess(reinterpret_cast<char*>(replacement))).get();

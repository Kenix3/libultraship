...
static void gfx_dxgi_swap_buffers_begin(void) {
    // This overrides the fractional VSync code by setting the VSync interval to a specific number.
    // It was causing the interval to change rapidly when using VRR displays (GSync or FreeSync),
    // causing a big hit to performance. Can be potentially set with a config option in the
    // future to disable/enable VSync entirely.
    // 0 = VSync off.
    // 1 = VSync on.
    dxgi.length_in_vsync_frames = 1;

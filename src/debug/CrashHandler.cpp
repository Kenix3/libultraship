...
static void printStack(CONTEXT* ctx, char* logBuffer, size_t* pos) {
    ...
    for (frame = 0;; frame++) {
        ...
        if (SymGetLineFromAddr(process, stack.AddrPC.Offset, &disp, &line)) {
            ...
            append_str(logBuffer, pos, symbol->Name);
            append_str(logBuffer, pos, " in ");
            append_str(logBuffer, pos, line.FileName);
            append_line(logBuffer, pos, lineNumberStr);
            ...
            WRITE_VAR(logBuffer, pos, "At ", symbol->Name);
            WRITE_VAR_LINE(logBuffer, pos, "Addr: ", addrString);

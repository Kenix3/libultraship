...
static void ErrorHandler(int sig, siginfo_t* sigInfo, void* data) {
    std::shared_ptr<CrashHandler> crashHandler = ShipDK::Context::GetInstance()->GetCrashHandler();
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, (ShipDK::Context::GetInstance()->GetName() + " has crashed").c_str(),
                             (ShipDK::Context::GetInstance()->GetName() +
    ...
    ShipDK::Context::GetInstance()->GetLogger()->flush();
    spdlog::shutdown();
    exit(1);
}

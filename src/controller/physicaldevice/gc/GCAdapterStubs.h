...
namespace Common
{
   ...
   public:
      ...
      void Set(bool value = true) { m_flag.store(value); }
      void Clear() { m_flag.store(false); }
      bool IsSet() const { return m_flag.load(); }
      bool TestAndClear() { return m_flag.exchange(false); }
   ...
   inline void SleepCurrentThread(int ms) { std::this_thread::sleep_for(std::chrono::milliseconds(ms)); }
   inline void YieldCPU() { std::this_thread::yield(); }

   template <int bit, typename T>
   constexpr bool ExtractBit(T val) { return (val >> bit) & 1; }
...
namespace LibusbUtils
{
   ...
   {
   public:
      Context() { libusb_init(&m_context); }
      ~Context() { libusb_exit(m_context); }
      operator libusb_context *() const { return m_context; }
   ...
   inline std::string ErrorWrap(int error_code) { return libusb_strerror(static_cast<libusb_error>(error_code)); }
}
...
namespace Config
{
   ...
   inline ConfigInfo GetInfoForSIDevice(int i) { return 0; }
   inline ConfigInfo GetInfoForAdapterRumble(int i) { return 0; }
   inline SerialInterface::SIDevices Get(ConfigInfo) { return SerialInterface::SIDEVICE_WIIU_ADAPTER; }
   inline bool Get(const ConfigInfo &, bool default_val) { return true; }
   inline ConfigChangedCallbackID AddConfigChangedCallback(std::function<void()>) { return 1; }
...
namespace Core
{
   ...
   struct SystemTimers
   {
      uint64_t GetTicksPerSecond() { return 1; }
   };
   ...
   inline State GetState(System &) { return State::Uninitialized; }
   inline bool WantsDeterminism() { return false; }
}
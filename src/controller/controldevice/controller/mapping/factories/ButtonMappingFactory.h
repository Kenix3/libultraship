...
class ButtonMappingFactory {
  public:
    ...
    static std::vector<std::shared_ptr<ControllerButtonMapping>> CreateDefaultKeyboardButtonMappings(uint8_t portIndex, uint16_t bitmask);
    ...
    CreateButtonMappingFromRawPress(uint8_t portIndex, uint16_t bitmask);
};
} // namespace LUS

...
void InputEditorWindow::UpdateElement() {
    ...
    if (mInputEditorPopupOpen && ImGui::IsPopupOpen("", ImGuiPopupFlags_AnyPopupId)) {
        Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->BlockGameInput(INPUT_EDITOR_WINDOW_GAME_INPUT_BLOCK_ID);
        ...
        if (mGameInputBlockTimer != INT32_MAX) {
            ...
            if (mGameInputBlockTimer <= 0) {
                Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->UnblockGameInput(INPUT_EDITOR_WINDOW_GAME_INPUT_BLOCK_ID);
                mGameInputBlockTimer = INT32_MAX;
            }
        }
...
void InputEditorWindow::DrawButtonLineEditMappingButton(uint8_t port, CONTROLLERBUTTONS_T bitmask, std::string id) {
    auto mapping =
        Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetButton(bitmask)->GetButtonMappingById(
            id);
    if (mapping == nullptr) {
        return;
    }
    ...
    if (sdlAxisDirectionToButtonMapping != nullptr) {
        ...
        if (ImGui::BeginPopup(popupId.c_str())) {
            ...
            auto globalSettings = Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetGlobalSDLDeviceSettings();
    ...
    if (ImGui::Button(StringHelper::Sprintf("%s###removeButtonMappingButton%s", ICON_FA_TIMES, id.c_str()).c_str(),
        ...
        Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetButton(bitmask)->ClearButtonMapping(id);
    };
    ImGui::PopStyleColor();
    ImGui::PopStyleColor();
    ImGui::PopStyleVar();
...
void InputEditorWindow::DrawStickSection(uint8_t port, uint8_t stick, int32_t id, ImVec4 color = CHIP_COLOR_N64_GREY) {
    ...
    if (stick == LEFT) {
        controllerStick = Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetLeftStick();
    } else {
        controllerStick = Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetRightStick();
    }
...
void InputEditorWindow::UpdateStickDirectionToMappingIds(uint8_t port) {
    ...
    for (auto stick :
         { std::make_pair<uint8_t, std::shared_ptr<ControllerStick>>(
               LEFT, Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetLeftStick()),
           std::make_pair<uint8_t, std::shared_ptr<ControllerStick>>(
               RIGHT, Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetRightStick()) }) {
...
void InputEditorWindow::DrawRemoveRumbleMappingButton(uint8_t port, std::string id) {
    ...
    if (ImGui::Button(StringHelper::Sprintf("%s###removeRumbleMapping%s", ICON_FA_TIMES, id.c_str()).c_str(),
        ...
        Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetRumble()->ClearRumbleMapping(id);
    }
    ImGui::PopStyleVar();
}
...
void InputEditorWindow::DrawRumbleSection(uint8_t port) {
    for (auto [id, mapping] :
         Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetRumble()->GetAllRumbleMappings()) {
...
void InputEditorWindow::DrawRemoveLEDMappingButton(uint8_t port, std::string id) {
    ...
    if (ImGui::Button(StringHelper::Sprintf("%s###removeLEDMapping%s", ICON_FA_TIMES, id.c_str()).c_str(),
                      ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f)))) {
        Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetLED()->ClearLEDMapping(id);
    }
    ImGui::PopStyleVar();
}
...
void InputEditorWindow::DrawLEDSection(uint8_t port) {
    for (auto [id, mapping] :
         Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetLED()->GetAllLEDMappings()) {
        ImGui::AlignTextToFramePadding();
        ImGui::SetNextItemOpen(true, ImGuiCond_Once);
...
void InputEditorWindow::DrawRemoveGyroMappingButton(uint8_t port, std::string id) {
    ...
    if (ImGui::Button(StringHelper::Sprintf("%s###removeGyroMapping%s", ICON_FA_TIMES, id.c_str()).c_str(),
                      ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f)))) {
        Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetGyro()->ClearGyroMapping();
    }
    ImGui::PopStyleVar();
}
...
void InputEditorWindow::DrawGyroSection(uint8_t port) {
    auto mapping = Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(port)->GetGyro()->GetGyroMapping();
    if (mapping != nullptr) {
...
void InputEditorWindow::DrawDeviceToggles(uint8_t portIndex) {
    ...
    auto connectedDeviceManager = Ship::Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetConnectedPhysicalDeviceManager();
...
void InputEditorWindow::DrawClearAllButton(uint8_t portIndex) {
    ...
    if (ImGui::BeginPopupModal("Clear All##clearAllPopup", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if (ImGui::Button("Clear All")) {
            Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(portIndex)->ClearAllMappings();
            ImGui::CloseCurrentPopup();
        }
        ImGui::EndPopup();
    }
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (ImGui::BeginPopupModal("Set Defaults for Keyboard", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            if (ImGui::Button("Set defaults")) {
                Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(portIndex)->ClearAllMappingsForDeviceType(
                    PhysicalDeviceType::Keyboard);
                Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(portIndex)->AddDefaultMappings(
                    PhysicalDeviceType::Keyboard);
                shouldClose = true;
                ImGui::CloseCurrentPopup();
            }
        ...
        if (ImGui::BeginPopupModal("Set Defaults for Gamepad (SDL)", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            if (ImGui::Button("Set defaults")) {
                ...
                Ship::Context::GetInstance()->GetChildren().GetFirst<ControlDeck>()->GetControllerByPort(portIndex)->AddDefaultMappings(

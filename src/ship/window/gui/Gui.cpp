...
void Gui::ImGuiWMInit() {
    switch (Context::GetInstance()->GetChildren().GetFirst<Window>()->GetWindowBackend()) {
...
void Gui::ApplyResolutionChanges() {
    ...
    const float aspectRatioX = Ship::Context::GetInstance()->GetChildren().GetFirst<ConsoleVariable>()->GetFloat(
...
int16_t Gui::GetIntegerScaleFactor() {
    if (!Ship::Context::GetInstance()->GetChildren().GetFirst<ConsoleVariable>()->GetInteger(
        ...
        int16_t factor = Ship::Context::GetInstance()->GetChildren().GetFirst<ConsoleVariable>()->GetInteger(
        ...
        factor += Ship::Context::GetInstance()->GetChildren().GetFirst<ConsoleVariable>()->GetInteger(
...
void Gui::DrawMenu() {
    ...
    if (ImGui::IsKeyPressed(TOGGLE_BTN, false) || ImGui::IsKeyPressed(ImGuiKey_Escape, false) ||
        ...
        if (!GetMenuOrMenubarVisible()) {
            Context::GetInstance()->GetChildren().GetFirst<Window>()->SetMouseCapture(wnd->ShouldAutoCaptureMouse());
        } else {
...
void Gui::CalculateGameViewport() {
    ...
    switch (Ship::Context::GetInstance()->GetChildren().GetFirst<ConsoleVariable>()->GetInteger(CVAR_LOW_RES_MODE, 0)) {
        case 1: { // N64 Mode
...
void Gui::DrawGame() {
    ...
    if (Ship::Context::GetInstance()->GetChildren().GetFirst<ConsoleVariable>()->GetInteger(CVAR_LOW_RES_MODE, 0) ==
        1) { // N64 Mode takes priority
        const float sw = size.y * 320.0f / 240.0f;
        pos = ImVec2(floor(size.x / 2 - sw / 2), 0);
        size = ImVec2(sw, size.y);
    } else if (Ship::Context::GetInstance()->GetChildren().GetFirst<ConsoleVariable>()->GetInteger(
    ...
    uintptr_t fb = Ship::Context::GetInstance()->GetChildren().GetFirst<Window>()->GetGfxFrameBuffer();
    if (fb) {
...
void Gui::DrawFloatingWindows() {
    if (mImGuiIo->ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
        WindowBackend backend = Context::GetInstance()->GetChildren().GetFirst<Window>()->GetWindowBackend();

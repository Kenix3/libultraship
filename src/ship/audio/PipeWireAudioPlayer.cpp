...
bool PipeWireAudioPlayer::DoInit() {
    ...
    char latencyStr[64];
    std::snprintf(latencyStr, sizeof(latencyStr), "%d/%d", GetSampleLength(), GetSampleRate());
    ...
    mStream =
        pw_stream_new_simple(pw_thread_loop_get_loop(mLoop), "libultraship",
                             pw_properties_new(PW_KEY_MEDIA_TYPE, "Audio", PW_KEY_MEDIA_CATEGORY, "Playback",
                                               PW_KEY_MEDIA_ROLE, "Game", PW_KEY_NODE_LATENCY, latencyStr, nullptr),
                             &sStreamEvents, this);
    ...
    if (pw_stream_connect(mStream, PW_DIRECTION_OUTPUT, PW_ID_ANY,
                          static_cast<pw_stream_flags>(PW_STREAM_FLAG_AUTOCONNECT | PW_STREAM_FLAG_MAP_BUFFERS), params,
                          1) < 0) {
    ...
    SPDLOG_INFO("PipeWire audio initialized: {} ch, {} Hz, latency hint {}", mNumChannels, GetSampleRate(), latencyStr);
    return true;
}
...
void PipeWireAudioPlayer::OnStateChanged(void* userdata, enum pw_stream_state /*old_state*/,
    ...
    if (new_state == PW_STREAM_STATE_ERROR) {
        SPDLOG_ERROR("PipeWire: stream error: {}", error ? error : "unknown");

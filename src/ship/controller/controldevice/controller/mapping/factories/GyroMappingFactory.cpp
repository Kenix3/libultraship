...
std::shared_ptr<ControllerGyroMapping> GyroMappingFactory::CreateGyroMappingFromConfig(uint8_t portIndex,
    ...
    float sensitivity = Ship::Context::GetInstance()->GetChild<ConsoleVariable>()->GetFloat(
    ...
    if (mappingClass == "SDLGyroMapping") {
        float neutralPitch = Ship::Context::GetInstance()->GetChild<ConsoleVariable>()->GetFloat(
            StringHelper::Sprintf("%s.NeutralPitch", mappingCvarKey.c_str()).c_str(), 0.0f);
        float neutralYaw = Ship::Context::GetInstance()->GetChild<ConsoleVariable>()->GetFloat(
            StringHelper::Sprintf("%s.NeutralYaw", mappingCvarKey.c_str()).c_str(), 0.0f);
        float neutralRoll = Ship::Context::GetInstance()->GetChild<ConsoleVariable>()->GetFloat(

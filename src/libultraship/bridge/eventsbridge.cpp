...
EventID EventSystemRegisterEvent(const char* name) {
    return Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::EventSystem>()->RegisterEvent(name);
}

ListenerID EventSystemRegisterListener(EventID id, EventCallback callback, EventPriority priority, const char* file,
                                       int line) {
    return Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::EventSystem>()->RegisterListener(id, callback,
                                                                                                       priority, file,
                                                                                                       line);
}

void EventSystemUnregisterListener(EventID ev, ListenerID id) {
    Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::EventSystem>()->UnregisterListener(ev, id);
}

void EventSystemCallEvent(EventID id, void* event, const char* file, int line, const char* key) {
    Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::EventSystem>()->CallEvent(

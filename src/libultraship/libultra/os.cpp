...
int32_t osContInit(OSMesgQueue* mq, uint8_t* controllerBits, OSContStatus* status) {
    *controllerBits = 0;
    ...
    Ship::Context::GetInstance()->GetChild<Ship::ControlDeck>()->Init(controllerBits);
...
void osContGetReadData(OSContPad* pad) {
    ...
    Ship::Context::GetInstance()->GetChild<Ship::ControlDeck>()->WriteToPad(pad);
}
...
int32_t __osMotorAccess(OSPfs* pfs, uint32_t vibrate) {
    auto io = Ship::Context::GetInstance()->GetChild<Ship::ControlDeck>()->GetControllerByPort(pfs->channel)->GetRumble();
    if (vibrate) {
        io->StartRumble();
    } else {
        io->StopRumble();

...
class ResourceManager {
    typedef enum class ResourceLoadError {
        None, NotCached, NotFound
    } ResourceLoadError;
...
public:
    ...
    LoadResource(const std::string &filePath, uintptr_t owner = 0, bool loadExact = false,
    ...
    LoadDirectory(const std::string &searchMask, uintptr_t owner = 0);

    std::shared_ptr<std::vector<std::shared_future<std::shared_ptr<IResource>>>>
    LoadDirectoryAsync(const std::string &searchMask, uintptr_t owner = 0,
                       BS::priority_t priority = BS::pr::normal);
...
protected:
    ...
    std::variant<ResourceLoadError, std::shared_ptr<IResource>> CheckCache(
            const std::string &filePath, uintptr_t owner = 0,
            bool loadExact = false);

private:
    std::unordered_map<uintptr_t, std::unordered_map<
            std::string, std::variant<ResourceLoadError, std::shared_ptr<IResource>>>> mResourceCache;

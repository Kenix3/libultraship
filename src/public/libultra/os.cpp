...
int32_t osContInit(OSMesgQueue* mq, uint8_t* controllerBits, OSContStatus* status) {
    *controllerBits = 0;
...
#ifndef __SWITCH__
    std::string controllerDb = ShipDK::Context::LocateFileAcrossAppDirs("gamecontrollerdb.txt");
    ...
    ShipDK::Context::GetInstance()->GetControlDeck()->Init(controllerBits);
...
void osContGetReadData(OSContPad* pad) {
    ...
    ShipDK::Context::GetInstance()->GetControlDeck()->WriteToPad(pad);
}

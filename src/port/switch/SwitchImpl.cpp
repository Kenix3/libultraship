...
void ShipDK::Switch::Init(SwitchPhase phase) {
    switch (phase) {
        case PreInitPhase:
            DetectAppletMode();
            break;
...
void ShipDK::Switch::ImGuiSetupFont(ImFontAtlas* fonts) {
...
void ShipDK::Switch::ImGuiProcessEvent(bool wantsTextInput) {
...
bool ShipDK::Switch::IsRunning() {
    return isRunning;
}

void ShipDK::Switch::GetDisplaySize(int* width, int* height) {
...
void ShipDK::Switch::ApplyOverclock(void) {
    SwitchProfiles perfMode = (SwitchProfiles)CVarGetInteger("gSwitchPerfMode", (int)ShipDK::MAXIMUM);

    if (perfMode >= 0 && perfMode <= ShipDK::POWERSAVINGM3) {
        if (hosversionBefore(8, 0, 0)) {
...
void ShipDK::Switch::PrintErrorMessageToScreen(const char* str, ...) {
    consoleInit(NULL);
    srand(time(0));
...
static void on_applet_hook(AppletHookType hook, void* param) {
    ...
    switch (hook) {
        ...
        case AppletHookType_OnFocusState:
            ...
            if (!hasFocus) {
                if (hosversionBefore(8, 0, 0)) {
                    pcvSetClockRate(PcvModule_CpuBus, SWITCH_CPU_SPEEDS_VALUES[ShipDK::STOCK]);
                } else {
                    ClkrstSession session = { 0 };
                    clkrstOpenSession(&session, PcvModuleId_CpuBus, 3);
                    clkrstSetClockRate(&session, SWITCH_CPU_SPEEDS_VALUES[ShipDK::STOCK]);
                    clkrstCloseSession(&session);
                }
            } else {
                ...
                ShipDK::Context::GetInstance()->GetAudio()->SetAudioBackend(ShipDK::AudioBackend::SDL);
            }
...
void DetectAppletMode() {
    ...
    ShipDK::Switch::PrintErrorMessageToScreen("\x1b[2;2HYou've launched the Ship while in Applet mode."
...
void ShipDK::Switch::ThrowMissingOTR(std::string OTRPath) {
    ShipDK::Switch::PrintErrorMessageToScreen("\x1b[2;2HYou've launched the Ship without the OTR file."

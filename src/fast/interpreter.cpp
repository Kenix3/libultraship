...
bool gfx_mtx_otr_filepath_handler_custom_f3dex2(F3DGfx** cmd0) {
    ...
    F3DGfx* cmd = *cmd0;
    const char* fileName = (const char*)cmd->words.w1;
    const int32_t* mtx = (const int32_t*)Ship::Context::GetInstance()
                             ->GetChildren()
...
bool gfx_movemem_handler_otr(F3DGfx** cmd0) {
    ...
    if (ucode_handler_index == ucode_f3dex2) {
        ...
        auto light = (Fast::LightEntry*)Ship::Context::GetInstance()
                         ->GetChildren()
                         .GetFirst<Ship::ResourceManager>()
                         ->GetResourceRawPointer(hash);
...
int16_t Interpreter::CreateShader(const std::string& path) {
    ...
    auto shader =
        Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::ResourceManager>()->GetArchiveManager()->LoadFile(
            path);
...
bool gfx_vtx_hash_handler_custom(F3DGfx** cmd0) {
    ...
    if (offset > 0xFFFFF) {
        ...
        F3DVtx* vtx = (F3DVtx*)Ship::Context::GetInstance()
                          ->GetChildren()
                          .GetFirst<Ship::ResourceManager>()
                          ->GetResourceRawPointer(hash);
...
bool gfx_vtx_otr_filepath_handler_custom(F3DGfx** cmd0) {
    ...
    F3DVtx* vtx =
        (F3DVtx*)Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::ResourceManager>()->GetResourceRawPointer(
            (const char*)fileName);
    vtx += vtxDataOff;
...
bool gfx_dl_otr_filepath_handler_custom(F3DGfx** cmd0) {
    ...
    char* fileName = (char*)cmd->words.w1;
    F3DGfx* nDL =
        (F3DGfx*)Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::ResourceManager>()->GetResourceRawPointer(
            (const char*)fileName);
...
bool gfx_dl_otr_hash_handler_custom(F3DGfx** cmd0) {
    ...
    if (C0(16, 1) == 0) {
        ...
        F3DGfx* gfx = (F3DGfx*)Ship::Context::GetInstance()
                          ->GetChildren()
                          .GetFirst<Ship::ResourceManager>()
                          ->GetResourceRawPointer(hash);
...
bool gfx_set_timg_otr_hash_handler_custom(F3DGfx** cmd0) {
    ...
    const char* fileName = Ship::Context::GetInstance()
                               ->GetChildren()
                               .GetFirst<Ship::ResourceManager>()
                               ->GetArchiveManager()
                               ->HashToCString(hash);
...
static void gfx_step() {
    ...
    if (cmd->words.trace.valid &&
        Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::ConsoleVariable>()->GetInteger("gEnableGFXTrace",
                                                                                                  0)) {
#define TRACE                                  \
    "\n====================================\n" \
    " - CMD: {:02X}\n"                         \
...
void Interpreter::Init(class GfxWindowBackend* wapi, class GfxRenderingAPI* rapi, const char* game_name,
    ...
    mCurDimensions.internal_mul =
        Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::ConsoleVariable>()->GetFloat(
            CVAR_INTERNAL_RESOLUTION, 1);
    mMsaaLevel =
        Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::ConsoleVariable>()->GetInteger(CVAR_MSAA_VALUE, 1);
...
int32_t gfx_check_image_signature(const char* imgData) {
    ...
    if (i != 0) {
        return Ship::Context::GetInstance()->GetChildren().GetFirst<Ship::ResourceManager>()->OtrSignatureCheck(
            imgData);
    }

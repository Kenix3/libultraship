...
void Interpreter::Gfxs2dexBgCopy(F3DuObjBg* bg) {
    ...
    */
    ...
    if ((bool)gfx_check_image_signature((char*)data)) {
        std::shared_future<std::shared_ptr<Ship::IResource>> future = mResourceManager->LoadResourceAsync((char*)data);
...
bool gfx_set_timg_handler_rdp(F3DGfx** cmd0) {
    ...
    if ((i & 1) != 1) {
        if (gfx_check_image_signature(imgData) == 1) {
            std::shared_future<std::shared_ptr<Ship::IResource>> future = sResourceManager->LoadResourceAsync(imgData);
...
bool gfx_set_timg_otr_hash_handler_custom(F3DGfx** cmd0) {
    ...
    std::shared_future<std::shared_ptr<Ship::IResource>> future = sResourceManager->LoadResourceAsync(fileName);
...
void Interpreter::RegisterBlendedTexture(const char* name, uint8_t* mask, uint8_t* replacement) {
    ...
    if (gfx_check_image_signature(reinterpret_cast<char*>(replacement))) {
        std::shared_future<std::shared_ptr<Ship::IResource>> future =
            mResourceManager->LoadResourceAsync(reinterpret_cast<char*>(replacement));

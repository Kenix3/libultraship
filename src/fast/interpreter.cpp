...
void Interpreter::GfxSpMatrix(uint8_t parameters, const int32_t* addr) {
    ...
    if (parameters & mtx_projection) {
        if (parameters & mtx_load) {
            ...
            mRsp->projection_is_perspective =
                std::abs(matrix[2][3]) > 0.0001f && std::abs(matrix[3][3]) < 0.0001f;
        } else {
...
void Interpreter::AdjustVIewportOrScissor(XYWidthHeight* area) {
    if (!mFbActive) {
        ...
        if (!mRendersToFb || (!IsStereoEnabled() && mMsaaLevel > 1 &&
...
void Interpreter::RunCommandPass(Gfx* commands, const std::unordered_map<Mtx*, MtxF>& mtxReplacements,
                                 StereoEye eye, bool enableDebugger) {
    SpReset();
...
StereoLayout Interpreter::GetStereoLayout() const {
    ...
    const double scale =
        std::max(static_cast<double>(mCurDimensions.width) / displayWidth,
                 static_cast<double>(mCurDimensions.height) / displayHeight);

    StereoLayout layout{};
    layout.eye_width =
        std::max(mCurDimensions.width, static_cast<uint32_t>(std::lround(displayWidth * scale)));
    layout.eye_height =
        std::max(mCurDimensions.height, static_cast<uint32_t>(std::lround(displayHeight * scale)));
...
void Interpreter::ComposeStereoEye(StereoEye eye) {
    ...
    mRapi->CopyFramebuffer(mStereoOutputFb, mGameFb, 0, 0, static_cast<int>(mCurDimensions.width),
                           static_cast<int>(mCurDimensions.height), dstX0, dstY0,
                           dstX0 + static_cast<int>(mCurDimensions.width),
                           dstY0 + static_cast<int>(mCurDimensions.height));
}
...
void Interpreter::CopyFrameBuffer(int fb_dst_id, int fb_src_id, bool copyOnce, bool* hasCopiedPtr) {
    ...
    if (fb_src_id == 0 || (!IsStereoEnabled() && mMsaaLevel > 1 &&
                           mCurDimensions.width == mGameWindowViewport.width &&
                           mCurDimensions.height == mGameWindowViewport.height)) {
...
void Interpreter::SetStereoMode(int32_t mode) {
    if (mode < static_cast<int32_t>(StereoMode::Off) ||
        mode > static_cast<int32_t>(StereoMode::FullTAB)) {
...
float Interpreter::GetStereoUiOffset() const {
    ...
    return static_cast<float>(static_cast<int8_t>(mStereoEye)) * std::min(mStereoSeparation * 0.0016f, mStereoUiDepth * 0.001f);
}

float Interpreter::GetStereoBackgroundOffset() const {
    return static_cast<float>(static_cast<int8_t>(mStereoEye)) * mStereoSeparation *
           STEREO_BACKGROUND_DEPTH_SCALE;
}
...
void Interpreter::ApplyStereoToClipCoordinates(float& x, float w) const {
    ...
    if ((mRsp->extra_geometry_mode & G_EX_STEREO_HORIZON) != 0) {
        ...
        const float horizonOffset = (mRsp->extra_geometry_mode & G_EX_STEREO_BACKGROUND) != 0
                                        ? GetStereoBackgroundOffset()
                                        : eyeOffset;
        x += horizonOffset * w;
        return;
    }
    ...
    x += eyeOffset * (w - mStereoConvergence * 2.0f);
}

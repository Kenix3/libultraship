...
bool gfx_mtx_otr_filepath_handler_custom_f3dex2(F3DGfx** cmd0) {
    ...
    F3DGfx* cmd = *cmd0;
    const char* fileName = (const char*)cmd->words.w1;
    const int32_t* mtx = (const int32_t*)mResourceManager
...
bool gfx_movemem_handler_otr(F3DGfx** cmd0) {
    ...
    if (ucode_handler_index == ucode_f3dex2) {
        gfx->GfxSpMovememF3dex2(
            index, offset,
            mResourceManager->GetResourceRawPointer(hash));
    } else {
        auto light = (Fast::LightEntry*)mResourceManager
                         ->GetResourceRawPointer(hash);
...
bool gfx_vtx_hash_handler_custom(F3DGfx** cmd0) {
    ...
    if (offset > 0xFFFFF) {
        ...
        F3DVtx* vtx = (F3DVtx*)mResourceManager
                          ->GetResourceRawPointer(hash);
...
bool gfx_vtx_otr_filepath_handler_custom(F3DGfx** cmd0) {
    ...
    F3DVtx* vtx =
        (F3DVtx*)mResourceManager->GetResourceRawPointer(
            (const char*)fileName);
    vtx += vtxDataOff;
...
bool gfx_dl_otr_filepath_handler_custom(F3DGfx** cmd0) {
    ...
    char* fileName = (char*)cmd->words.w1;
    F3DGfx* nDL =
        (F3DGfx*)mResourceManager->GetResourceRawPointer(
            (const char*)fileName);
...
bool gfx_dl_otr_hash_handler_custom(F3DGfx** cmd0) {
    ...
    if (C0(16, 1) == 0) {
        ...
        F3DGfx* gfx = (F3DGfx*)mResourceManager
                          ->GetResourceRawPointer(hash);
...
bool gfx_set_timg_otr_hash_handler_custom(F3DGfx** cmd0) {
    ...
    const char* fileName = mResourceManager
                               ->GetArchiveManager()
                               ->HashToCString(hash);
    uint32_t texFlags = 0;
    RawTexMetadata rawTexMetadata = {};
...
bool gfx_set_timg_otr_filepath_handler_custom(F3DGfx** cmd0) {
    ...
    std::shared_ptr<Fast::Texture> texture = std::static_pointer_cast<Fast::Texture>(
        mResourceManager->LoadResourceProcess(fileName));
    if (texture != nullptr) {
        Interpreter* gfx = mInstance.lock().get();
        texFlags = texture->Flags;
        rawTexMetadata.width = texture->Width;
...
int32_t gfx_check_image_signature(const char* imgData) {
    ...
    return mResourceManager->OtrSignatureCheck(imgData);
}
